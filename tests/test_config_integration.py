...
from soft_ue_cli.__main__ import _search_config
...
def test_search_config_finds_matching_key(ue_project):
    engine, project = ue_project
    discovery = ConfigDiscovery(engine_path=engine, project_path=project)
    matches = _search_config(discovery, "Bloom", "Engine", None)
    assert any(
        match["section"] == "/Script/Engine.RendererSettings"
        and match["key"] == "r.DefaultFeature.Bloom"
        and match["value"] == "True"
