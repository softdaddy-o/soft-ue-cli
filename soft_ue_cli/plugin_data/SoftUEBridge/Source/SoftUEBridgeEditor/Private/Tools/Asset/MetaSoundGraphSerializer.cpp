...
namespace
{
	/** Resolve a node's class name ("Namespace.Name" / variant) from the document dependency table. */
	...
	TSharedPtr<FJsonObject> LiteralToJson(const FMetasoundFrontendVertexLiteral& Literal)
	{
		...
		// FMetasoundFrontendLiteral exposes GetType()/ToString() — emit both rather than a
		// hand-rolled union switch over the 12 literal type variants.
...
namespace MetaSoundGraphSerializer
{
	TSharedPtr<FJsonObject> SerializeDocument(const FMetasoundFrontendDocument& Document, const FString& AssetType)
	{
		...
		// Declared graph interface (the asset's own inputs/outputs) lives on the root graph class.
